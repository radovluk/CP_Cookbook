#!/usr/bin/env python3
"""
RCPSP-AS Benchmark Runner

Compares OptalCP (Python) vs IBM CP Optimizer (Python/DOcplex) on RCPSP-AS instances.
Runs instances from the ASLIB dataset with configurable time limit per instance.

Configuration:
    - SOLVER_PYTHON: Path to Python with both OptalCP and DOcplex installed
    - TIME_LIMIT: Seconds per instance (default 60)
    - WORKERS: Number of parallel workers (default 8)
    - DATA_DIR: Path to RCPSP-AS instances directory

Usage:
    python run_benchmark_rcpspas.py                 # Run all instances
    python run_benchmark_rcpspas.py --max 1         # Run only 1 instance (test mode)
    python run_benchmark_rcpspas.py --max 10        # Run first 10 instances
"""
...
# Data directory (relative to script location or absolute path)
DATA_DIR = "../../data/rcpspas"
...
def collect_instances(data_dir):
    """Collect all RCPSP-AS instances (.rcp files) from data directory."""
    instances = []
    
    # Recursively find all .rcp files
    for root, dirs, files in os.walk(data_dir):
        for file in files:
            if file.endswith('.rcp'):
                instances.append(os.path.join(root, file))
    
    # Also try direct glob patterns
    patterns = [
        "*.rcp",
        "*/*.rcp",
        "*/*/*.rcp",
...
def categorize_instances(instances):
    """Group instances by dataset/folder."""
    categories = {}
    
    for inst in instances:
        # Get the parent folder name as category
        parent = os.path.basename(os.path.dirname(inst))
        if parent not in categories:
            categories[parent] = []
        categories[parent].append(inst)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='RCPSP-AS Benchmark Runner',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_benchmark_rcpspas.py                 # Run all instances
  python run_benchmark_rcpspas.py --max 1         # Test mode: run only 1 instance
  python run_benchmark_rcpspas.py --max 10        # Run first 10 instances
  python run_benchmark_rcpspas.py --max 5 --timeLimit 30  # 5 instances, 30s each
        """
    )
    ...
    data_dir_candidates = [
        ...
        os.path.join(base_dir, "data/rcpspas"),
        os.path.join(base_dir, "../data/rcpspas"),
        os.path.join(base_dir, "../../data/rcpspas"),
        DATA_DIR,  # Absolute path if provided
    ]
    ...
    if data_dir is None:
        ...
        for c in data_dir_candidates:
            print(f"  - {c}")
        print("\nPlease update DATA_DIR in the script configuration.")
        return
    ...
    if not instances:
        ...
        print("Expected .rcp files in the data directory")
        if os.path.exists(data_dir):
            ...
            if contents:
                print(f"Sample files found: {contents}")
        return
    ...
    print("="*70)
    print("RCPSP-AS Benchmark Runner")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    for category, cat_instances in sorted(categories.items()):
    ...
    optalcp_results_file = os.path.join(results_dir, "optalcp-rcpspas-results.json")
    ...
    cpo_results_file = os.path.join(results_dir, "cpo-rcpspas-results.json")
    ...
    comparison_dir = os.path.join(results_dir, "comparison-report-rcpspas")
    compare_tool = os.path.join(base_dir, "../compare/compare.mjs")
    
    if os.path.exists(compare_tool):
        cmd = (f'node "{compare_tool}" "RCPSP-AS Benchmark" '
        ...
        print("Skipping comparison report generation")
